...
loopVars = expand.grid(the_scale = c(2,5,10,20,40),doStandardization=c(F,T))
#note: figs do not print properly in loop. Step through manually.
loopVars
l = 10
  doStandardization=loopVars$doStandardization[l]
  the_scale = loopVars$the_scale[l]
  ...
  png(paste0("outputs/figures","/fig5MapsRawBig",selectTerm,"std",doStandardization,".png"),
      width=8,height=5,units="in",res=3*72)
  par(mar=c(0,0,0,0), oma=c(0,0,0,0))
